...
COMPANY = 'c:\\QuickBooks\MyFile.QBW'.freeze
QBWC_USERNAME = 'myUserName'.freeze
QBWC_PASSWORD = 'myPassword'.freeze
QBWC.api = :qb
...
def _inspect_routes
  ...
  Rails.application.routes.routes.each do |route|
    puts "  Name #{route.name}: \t #{route.verb.source.gsub(/[$^]/, '')} #{route.path.spec}"
  end
  puts "\n"
end
...
module QbwcTestApplication
  class Application < Rails::Application
    ...
    QBWC.configure do |c|
      ...
      c.session_initializer = proc { |_session| $CONFIG_SESSION_INITIALIZER_PROC_EXECUTED = true }
    end
...
QBWC_CUSTOMER_QUERY_RESPONSE_INFO = "<?xml version=\"1.0\" ?><QBXML><QBXMLMsgsRs><CustomerQueryRs statusCode=\"0\" statusSeverity=\"Info\" statusMessage=\"Status OK\"><CustomerRet><ListID>8000001B-1405777862</ListID><TimeCreated>2014-07-30T21:11:20-05:00</TimeCreated><TimeModified>2005-07-30T21:11:20-05:00</TimeModified><EditSequence>1405777862</EditSequence><Name>#{QBWC_USERNAME}</Name><Salutation>Mr</Salutation><FullName>#{QBWC_USERNAME}</FullName><IsActive>true</IsActive><Sublevel>0</Sublevel><Email>#{QBWC_USERNAME}@gmail.com</Email><Balance>0.00</Balance><TotalBalance>0.00</TotalBalance><AccountNumber>123456789</AccountNumber><JobStatus>None</JobStatus></CustomerRet></CustomerQueryRs></QBXMLMsgsRs></QBXML>".freeze

QBWC_CUSTOMER_QUERY_STATUS_MESSAGE_WARN = 'The query request has not been fully completed. There was a required element ("bleech") that could not be found in QuickBooks.'.freeze

QBWC_CUSTOMER_QUERY_RESPONSE_WARN = '<?xml version="1.0" ?><QBXML><QBXMLMsgsRs><CustomerQueryRs statusCode="500" statusSeverity="Warn" statusMessage="The query request has not been fully completed. There was a required element (&quot;bleech&quot;) that could not be found in QuickBooks." /></QBXMLMsgsRs></QBXML>'.freeze

QBWC_CUSTOMER_QUERY_STATUS_MESSAGE_ERROR = 'Object "8000001B-1405768916" specified in the request cannot be found.  QuickBooks error message: Invalid argument.  The specified record does not exist in the list.'.freeze

QBWC_CUSTOMER_QUERY_RESPONSE_ERROR = '<?xml version="1.0" ?><QBXML><QBXMLMsgsRs><CustomerQueryRs statusCode="3120" statusSeverity="Error" statusMessage="Object &quot;8000001B-1405768916&quot; specified in the request cannot be found.  QuickBooks error message: Invalid argument.  The specified record does not exist in the list." /></QBXMLMsgsRs></QBXML>'.freeze
...
def _set_controller_env_if_required
  return unless _controller_env_is_required?
...
def _simulate_soap_request(http_action, soap_action, soap_params)
  ...
  unless session.blank?
    ...
    soap_params.update(ticket: ticket)
  end
