...
class QBWCControllerTest < ActionController::TestCase
  def setup
    ...
    QBWC.on_error = :stop
    QBWC::ActiveRecord::Session::QbwcSession.all.each(&:destroy)
    QBWC.clear_jobs
  ...
  test 'qwc' do
    ...
    assert_match(%r{AppURL.*http://test.host/qbwc/action.*AppURL}, @response.body)
    assert_match(/AppDescription.*Quickbooks integration.*AppDescription/, @response.body)
    assert_match(%r{AppSupport.*https://test.host/.*AppSupport}, @response.body)
  ...
  test 'most recent initialization block is executed' do
    ...
    QBWC.set_session_initializer do |_session|
  ...
  class CheckErrorValuesWorker < QBWC::Worker
    ...
    def handle_response(_response, session, _job, _request, expected)
  ...
  def _receive_response_error_helper(receive_response_xml_result, schedule_second_job)
    ...
    assert_match(/tns:receiveResponseXMLResult.*#{receive_response_xml_result}..tns:receiveResponseXMLResult/,
                 @response.body)
  end
  ...
  test 'session_complete_success block not called upon failed completion' do
    block_called = false
    QBWC.on_error = :stop
    QBWC.session_complete_success = lambda do |_session|
      block_called = true
    end
